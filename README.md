...
旅行规划是一个面向中文旅行场景的 AI 旅行规划项目。用户输入目的地、日期、预算、人数和偏好后，系统会自动生成结构化旅行方案，并进一步补充地图点位、天气信息、预算拆分、景点图片与可导出的旅行文档。
...
<details>
<summary><strong>查看版本更新记录（最新：2026-09-05）</strong></summary>

- `2026-09-5`
  - 同步改异步：RAG 检索 / 重排 / 改写、地图、天气、Redis、数据库全链路异步化（httpx.AsyncClient、ainvoke、redis.asyncio、SQLAlchemy 异步引擎），Chroma 同步调用放入 asyncio.to_thread，导出链路用 asyncio.to_thread 包住同步 PDF / 图片下载
  - RAG：知识库扩展为北京、大理、成都、西安、厦门、三亚、桂林 7 个目的地
  - 城市可以模糊输入 比如：上海 可以输入魔都 成都，可以输入：以大熊猫出名的城市
- `2026-07-26`
...
> **数据边界**：7 个本地 Markdown 攻略用于 RAG 参考；动态城市的地点实体来自当次高德 POI 候选。两条路径都不代表门票、酒店价格、营业状态或可预订性已经实时核验，相关金额目前属于规划估算。
...
- 📚 **本地攻略检索**：覆盖北京、大理、成都、西安、厦门、三亚、桂林 7 个目的地，为行程生成补充对应城市的攻略信息，并避免跨城市内容混入
...
- ✅ **RAG 检索增强**
  ...
  - ✅ 知识库已覆盖 7 个目的地，评估样例集 18 条；规则、fallback、评估断言与目的地 metadata 已有离线一致性校验。模型账户恢复后需重新执行真实 RAG 评估，建立新的在线质量基线。
- 🚧 **Token 成本分析看板**
...
- 🚧 **性能与稳定性**
  - ✅可以加入异步任务队列、
  请求限流、失败重试、日志追踪与监控告警，提升真实部署场景下的稳定性。
- 🚧 **产品能力延展**
